...
\subsection{Accuracy}
We would like to convert the error into an accuracy estimate. Keeping in line with existing standards in PSI-Library, we define the accuracy $a$ for a given $\alpha$ as the $a$ such that $\alpha = \mathbb{P}(N > a)$, where $N$ is (as above) the error (relative to $f(D)$) introduced by the Snapping Mechanism. It is difficult to get an exact accuracy guarantee because of our inability to model $\Lambda$ exactly, so we will instead try to represent $a$ such that $\mathbb{P}(N > a) \leq \alpha$. \newline
...
Let's attempt to get an upper bound on $\mathbb{P}(N = B + f(D))$ and $\mathbb{P}(N = B - f(D))$ in terms of quantities we know. Remember that $\frac{\lambda'}{2} \leq \frac{\Lambda}{2} < \lambda'$.
\begin{align}
...
\begin{align}
	P_{U} &= \mathbb{P}(N = B - f(D)) \nonumber \\
		  ...
		  &\leq 1 - F_{Y'}(B - f(D) - \frac{\lambda'}{2}) \nonumber \\
		  &= P_{U}^{+} \nonumber
\end{align}
Because we know that $N \leq P_L < P_{L}^{+}$, we can reframe our original goal. We wanted to find $a$ such that $\alpha \geq \mathbb{P}(N > a)$, but this is equivalent to finding $a$ such that
\[ \alpha - P_{L}^{+} \geq \mathbb{P}(N_{sub} > a) \]
which we further reinterpret as
\[ \alpha - P_{L}^{+} \geq 1 - \mathbb{P}(N_{sub} \leq a) = 1 - F_{N_{sub}}(a). \]
...
We now examine the CDF of $Z = \vert Y' \vert + \frac{\Lambda}{2}$, which we will call $F_{Z}$:
\begin{align}
...
\end{align}
Let the PDF of $Z$ be $f_Z$, $m = \max\left(B-f(D),a\right)$, and $n = \min\left(B+f(D),a\right)$.  Then we have:
\begin{align}
	F_{N_{sub}}(a) &= \int_{0}^{m} f_{Z}(z)dz + P_{U} \mathbbm{1}\left(a \geq B - f(D)\right) + \int_{B - f(D)}^{n} f_{Z}(z)dz \nonumber \\
				   &= F_{Z}(m) + P_{U} \mathbbm{1}(m = a) + F_{Z}(n) - F_{Z}(B - f(D)) \nonumber
\end{align}
\end{document}

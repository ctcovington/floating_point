...
For now, we will mostly ignore exactly how this is done and focus on general analysis assuming that such a $B$ can be chosen.
Our current method is to set $B = \max(\vert f(D) \vert) + \frac{1}{\epsilon'} \ln \left( \frac{1}{\gamma} \right)$ for some
$\gamma \in (0,1]$, which implies that $B$ does not bind with probability $1 - \gamma$.
\textbf{Victor told us about this argument, but we have not yet proven it.} \newline

...
% \textbf{NOTE:} \newline
% I am not sure how to do this in an actual implementation.
% We need to set $B$ before finding $\Lambda'$, as $\Lambda', \lambda', \epsilon'$ are functions of $B$.
% But then we cannot use $\Lambda'$ to define $B$. Note that
% \[ \Lambda' \leq 2\lambda' = \frac{2 + 24B\eta}{\epsilon - 2\eta}. \]
% For $\eta, B$ such that $B\eta << 1$,
% \textbf{KEEP WORKING ON THIS!}
